...
Errors:
  ...
  async (params: z.infer<typeof LaunchInputSchema>) => {
    refreshState();
...
Example:
  ...
  async (params: z.infer<typeof TypeInputSchema>) => {
    refreshState();
...
const KeyInputSchema = z
  .object({
    key: z
      .enum(SUPPORTED_KEYS as [string, ...string[]])
...
Examples:
  ...
  async (params: z.infer<typeof KeyInputSchema>) => {
    refreshState();
