...
export const state: VeloTermState = {
  pid: null,
  windowId: null,
  launchedAt: null,
};
...
export function buildVeloTerm(release: boolean): void {
...
export function createAppBundle(release: boolean): void {
...
export function findPid(): number | null {
  try {
 ...
 */
export async function waitForReady(): Promise<{
  pid: number;
  windowId: number;
  elapsedMs: number;
}> {
  ...
  throw new Error(
    `VeloTerm window did not appear within ${LAUNCH_TIMEOUT_MS / 1000}s. ` +
      `Check ${RUNTIME_LOG_PATH} for errors.`
  );
}

/** Kill VeloTerm and clear state. */
export function killVeloTerm(): boolean {
