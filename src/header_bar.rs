...
/// Height of the title bar in physical pixels (per VeloTerm spec).
pub const HEADER_BAR_HEIGHT: f32 = 38.0;
/// Height of the chrome bar in physical pixels (per VeloTerm spec).
pub const CHROME_BAR_HEIGHT: f32 = 38.0;
...
pub fn generate_header_bar_text_cells(
    ...
    // Left side: sparkle icon (✻) in accent + space + "VeloTerm" in primary text
    let accent = color_new(theme.accent_orange.r, theme.accent_orange.g, theme.accent_orange.b, 1.0);
    let text_color = color_new(theme.text_primary.r, theme.text_primary.g, theme.text_primary.b, 1.0);
    let dim_color = color_new(theme.text_ghost.r, theme.text_ghost.g, theme.text_ghost.b, 1.0);
...
mod tests {
    ...
    fn header_bar_height_constant() {
        assert_eq!(HEADER_BAR_HEIGHT, 38.0);
    }
    ...
    fn header_bar_text_contains_brand() {
        ...
        let brand: String = cells[4..12].iter().map(|c| c.ch).collect();
        assert_eq!(brand, "VeloTerm");
    }
