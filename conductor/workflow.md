...
# Install dependencies (handled automatically by cargo)
cargo fetch
```
...
- Use `#[cfg(test)]` module within source files or separate test files in `tests/`.
...
- Verify PTY interactions
- Test terminal emulation correctness
- Check configuration loading and hot-reload

### Performance Testing

- Benchmark input latency (target: <10ms)
- Benchmark throughput (cat large files)
- Benchmark startup time (target: <100ms)
- Monitor memory usage per pane
...
4. **Security**
   ...
   - PTY interactions sanitized

5. **Performance**
   - GPU operations optimized
   - No unnecessary allocations in hot paths
   - Damage tracking working correctly
   - Channel communication efficient
...
```bash
git commit -m "feat(renderer): Add glyph atlas with ASCII pre-rasterization"
git commit -m "fix(pty): Correct read buffer size for high-throughput output"
git commit -m "test(pane_tree): Add tests for nested split and close operations"
git commit -m "refactor(terminal): Extract adapter layer for alacritty_terminal"
```
...
- [ ] Binary tested on macOS Apple Silicon
- [ ] Binary tested on CentOS 9 X11
...
4. Create platform packages (.app bundle, RPM/AppImage)
5. Run smoke tests on release builds
6. Publish release
